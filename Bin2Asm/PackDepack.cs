...
public static class PackDepack {
	...
	static public int Pack(byte[] bufIn, int lengthIn, byte[] bufOut, int offsertOut) {
		...
		while (true) {
			...
			if (bitcount == 8) {
				bufOut[offsertOut++] = bits;
				System.Array.Copy(codebuffer, 0, bufOut, offsertOut, codecount);
				offsertOut += codecount;
		...
		bufOut[offsertOut++] = (byte)(bits | (1 << bitcount));
		System.Array.Copy(codebuffer, 0, bufOut, offsertOut, codecount);
		return (offsertOut + codecount);
	}
}
